...
public class MainWindow extends JFrame {
	...
	public JPanel SælgerMainPanel() {
		...
		btnOpretLåneanmodning
				.addActionListener(event -> new KreditværdighedDialog());
	...
	private JPanel FindKundePanel() {
		...
		comboString = new String[] { "CprID", "Navn", "Telefonnummer",
				"Adresse", "Email" };
	...
	private JPanel FindBilPanel() {
		...
		comboString = new String[] { "Chassisnummer", "Registreringsnummer",
				"Model" };
	...
	private JPanel FindLåneaftalePanel() {
		...
		comboString = new String[] { "AftaleID", "CprID", "Cprnummer",
				"Chassisnummer", "Registreringsnummer", "SælgerID" };
	...
	public void searchButtonPressed() {
		try {
			...
			if (this.getTitle() == "Kunde") {
				columnNames = new String[] { "Cpr-ID", "Navn", "Adresse",
						"Postnummer", "Telefon", "Email", "Kommentar" };
				kundesearchlist = kl.listKunder(comboBox.getSelectedItem()
						.toString(), searchField.getText());
				...
				columnNames = new String[] { "Chassisnummer", "Reg-nummer",
						"Pris", "Model", "Årgang", "Bemærkninger" };
				bilsearchlist = bl.listBiler(comboBox.getSelectedItem()
						.toString(), searchField.getText());
				...
				columnNames = new String[] { "Aftale-ID", "Oprettelsesdato",
						"Cpr-ID", "Beløb", "Udbetaling", "Rente",
						"Afviklingsperiode", "Bil-Chassisnummer", "SælgerID" };
				...
				aftalesearchlist = fl.listFinansieringsaftaler(comboBox
	...
	private JPanel ButtonPanel() {
		...
		redigerButton.addActionListener(event -> {
			if (this.getTitle() == "Kunde") {
				...
				kunde.setCprid(Integer.parseInt(table.getValueAt(
						table.getSelectedRow(), 0).toString()));
				kunde.setNavn(table.getValueAt(table.getSelectedRow(), 1)
						.toString());
				kunde.setAdresse(table.getValueAt(table.getSelectedRow(), 2)
						.toString());
				kunde.setPostnummer(table.getValueAt(table.getSelectedRow(), 3)
						.toString());
				kunde.setTelefonnummer(table.getValueAt(table.getSelectedRow(),
						4).toString());
				kunde.setEmail(table.getValueAt(table.getSelectedRow(), 5)
						.toString());
				kunde.setKommentar(table.getValueAt(table.getSelectedRow(), 6)
						.toString());
				new EditDialog(kunde);
			} else if (this.getTitle() == "Bil") {
				Bil bil = new Bil();
				bil.setChassisnummer(table
						.getValueAt(table.getSelectedRow(), 0).toString());
				bil.setRegistreringsnummer(table.getValueAt(
						table.getSelectedRow(), 1).toString());
				bil.setPris(Double.parseDouble(table.getValueAt(
						table.getSelectedRow(), 2).toString()));
				bil.setModel(table.getValueAt(table.getSelectedRow(), 3)
						.toString());
				bil.setÅrgang(table.getValueAt(table.getSelectedRow(), 4)
						.toString());
				bil.setBemærkninger(table.getValueAt(table.getSelectedRow(), 5)
						.toString());
				new EditDialog(bil);
			} else if (this.getTitle() == "LåneAftale") {
				Finansieringsaftale aftale = new Finansieringsaftale();
				aftale.setAftaleid(Integer.parseInt(table.getValueAt(
						table.getSelectedRow(), 0).toString()));
				aftale.setOprettelsesdato(table.getValueAt(
						table.getSelectedRow(), 1).toString());
				aftale.setCprid(Integer.parseInt(table.getValueAt(
						table.getSelectedRow(), 2).toString()));
				aftale.setBeløb(Double.parseDouble(table.getValueAt(
						table.getSelectedRow(), 3).toString()));
				aftale.setUdbetaling(Double.parseDouble(table.getValueAt(
						table.getSelectedRow(), 4).toString()));
				aftale.setRente(Double.parseDouble(table.getValueAt(
						table.getSelectedRow(), 5).toString()));
				aftale.setAfviklingsperiode(Integer.parseInt(table.getValueAt(
						table.getSelectedRow(), 6).toString()));
				aftale.setChassisnummer(table.getValueAt(
						table.getSelectedRow(), 7).toString());
				aftale.setSælgerid(Integer.parseInt(table.getValueAt(
						table.getSelectedRow(), 8).toString()));
